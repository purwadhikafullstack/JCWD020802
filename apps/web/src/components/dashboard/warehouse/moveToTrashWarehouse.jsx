...
export function MoveToTrashWarehouse({ warehouse, handleWarehouseUpdate }) {
    ...
    const handleMoveToTrash = async () => {
        ...
        try {
            setIsLoading(true)
            await Axios.patch(`warehouses/move-to-trash/${warehouse.id}`, {}, config)
            handleOpen()
            ...
            if (error.response.status == 400) {
                ...
                toast.error("Warehouse doesn't exist!")
            } else {
                toast.error("Failed moving warehouse data to trash!")
            } 
        } finally {
            setIsLoading(false)
        }
            ...
            <Dialog open={open} handler={handleOpen} className="flex flex-col max-h-full overflow-y-scroll">
                ...
                <DialogBody>
                    <Typography variant="lead">
                        Are you sure you want to move this Warehouse data to Trash?
                    </Typography>
                </DialogBody>
