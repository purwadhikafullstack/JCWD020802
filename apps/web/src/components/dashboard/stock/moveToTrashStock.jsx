...
export function MoveToTrashStock({ stock, handleStockUpdate }) {
    ...
    const handleMoveToTrash = async () => {
        ...
        try {
            setIsLoading(true)
            await Axios.patch(`stocks/move-to-trash/${stock.id}`, {}, config)
            handleOpen()
            handleStockUpdate()
            toast.success('Stock moved to trash!')
        } catch (error) {
            handleOpen()
            if (error.response.status == 400) {
                ...
                toast.error("Failed moving Stock data to trash!")
            } 
        } finally {
            setIsLoading(false)
        }
            ...
            <Dialog open={open} handler={handleOpen} className="flex flex-col max-h-full overflow-y-scroll">
                ...
                <DialogBody>
                    <Typography variant="lead">
                        Are you sure you want to move this Stock data to Trash?
                    </Typography>
                </DialogBody>
